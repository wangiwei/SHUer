...
def fitness(z, d, f, demand, facility, level, h3, capacity):
    """
    计算适应度函数，返回该代种群适应度值
    :param z: I*K 容量分配方案
    :param d: I*J*K 配送距离
    ...
    for i in range(0, demand):
        ...
        if z[i*2] == 180: # 首层未分配
            ...
            dis_cost = dis_cost + d[i, int(z[i*2+1]), 1]
            x[int(z[i*2+1])] = 1
        else:
            # print('----------z[', ',', i, ']----------', z[i*2])
            dis_cost = dis_cost + d[i, int(z[i*2]), 0]
            x[int(z[i*2])] = 1
    ...
    for j in range(facility):
        ...
        fitness_value = dis_cost + fac_cost + unfirst_count + 10 * punishment_over_capacity
